...
Subpackages:
  ...
  Benchmarking -- Classes for benchmarking warriors

Classes:

  Warrior -- A class for representing a Core War program
  SimpleParser -- Class for parsing warriors (simplified format only)
...
class Warrior(object):
    ...
    def __repr__(self):
        ...
        # Show minimal header which can be parsed without errors or warnings.
        s = ";redcode\n"
        s += ";name %s\n" % self.name
        s += ";author %s\n" % self.author
        s += ";assert 1\n\n"
        ...
        # Show PIN.
        if self.pin and self.standard == STANDARD_94:
            s += "       PIN      %s\n\n" % self.pin

        # Show start instruction for non ICWS '88 warriors.
        ...
        # Show start instruction for ICWS '88 warriors.
...
class SimpleParser(object):
    """Class for parsing warriors (simplified format only)."""

    coresize = 8000
    standard = STANDARD_94

    def __init__(self, coresize=8000, standard=STANDARD_94):
        """Set up the parser."""
    ...
    def parse_file(self, fname, maxsize=10000):
        """Parse warrior from file. Raise an exception, if the file is too
        big (default for max. allowed size: 10000 bytes)."""

        # Open file and check max. allowed size to prevent denial of service
        # attacks when used for a hill sever.
        # Use 'rb' as mode to circumvent some problem on Windows with tell().
        f = open(fname, "rb")
        f.seek(0, 2)
        fsize = f.tell()
        f.seek(0, 0)
    ...
    def parse(self, s, maxsize=10000):
        """parse(s) -> warrior
        ...
        for line in lines:
            ...
            # Handle ;redcode
            if lline.startswith(";redcode"):
                hillkey = line[8:].strip()
                if hillkey.startswith("-"):
                    hillkey = hillkey[1:].strip()
                warrior.hillkey = hillkey
                continue
            ...
            # Handle END.
            if lline.startswith("end"):
                continue

            # Handle PIN.
            if lline.startswith("pin"):
                if self.standard == STANDARD_94:
                    try:
            ...
            if lline.startswith("start"):
                warrior.start = len(warrior.instructions)
                line = line[5:]
...
class Parser(object):
    ...
    __INSN_RX = re.compile(
        "(?P<opcode>dat|mov|add|sub|jmp|jmz|jmn|djn|cmp|"
    ...
    def __init__(
        ...
        if mindistance < maxlength:
            raise ValueError(
                "Minimal distance between warriors is lower "
        ...
        self.__predefinedLabels = {
            "CORESIZE": self.coresize,
    ...
    def parse(self, s):
        ...
        for (
            ...
            if opcode in ("org", "end"):
                if self.standard == STANDARD_88 and opcode == "org":
                    raise WarriorParseError(
                        "Pseudoopcode ORG not allowed under '88 rules", lineNum
                    )
                if not afield == "":
            ...
            if opcode == "pin":
                if self.standard == STANDARD_88:
                    raise WarriorParseError(
                        "Pseudoopcode PIN not allowed under '88 rules", lineNum
                    )
                elif self.standard == STANDARD_94_NOP:
                    raise WarriorParseError(
                        "Pseudoopcode PIN not allowed under '94nop rules",
                        lineNum,
                    )
                try:
                    pin = int(afield)
            ...
            if self.standard == STANDARD_88:
                ...
                if self.standard == STANDARD_94_NOP and opcode in (
                    "stp",
                    "ldp",
                ):
                    raise WarriorParseError(
                        "Opcodes STP and LDP are not "
        ...
        if not self.__foundAssert:
            self.warnings.append(
                "Missing ';assert'. Warrior may not work "
        ...
        if len(warrior.instructions) > self.maxlength:
            ...
            raise WarriorParseError(
                "Too many instructions (about %d more)" % diff, lineNum
            )
    ...
    def __run_pass1(self):
        ...
        if pseudoopcode == "equ":
            ...
            # Get loop expression.
            loopExpr = self.__get_expression(
                line, pos + 3, self.__EXPR_CHARS, lineNum
            )
            ...
            # Get expression.
            startExpr = self.__get_expression(
                line, pos + 3, self.__EXPR_CHARS, lineNum
            )
            self.__pass1.append(
                (lineNum, pseudoopcode, "", "", startExpr, "", "")
            )
            if pseudoopcode == "end":
                return True
            else:
    ...
    def __get_expression(self, line, pos, validChars, lineNum):
        ...
        while pos < len(line):
            ...
            if not char in validChars:
                raise WarriorParseError(
                    "Invalid character '%s'" % char, lineNum
                )
            expr += char
    ...
    def __run_pass1_labels(self, line, pos, lineNum):
        ...
        while 1:
            ...
            if label in self.__tmpLabels or label in self.__labels:
                self.warnings.append(
                    "Warnining in line %d: Redefinition of "
                    "label '%s'" % (lineNum, label)
                )
            elif label in self.__equs:
                raise WarriorParseError("Redefinition of '%s'" % label, lineNum)
            else:
    ...
    def __run_pass1_assert(self, line, lineNum):
        ...
        if not expr:
            self.warnings.append(
                "Missing parameter for assertion in line %d" % lineNum
            )
            return False
        ...
        if not result == 1:
            raise WarriorParseError("Assertion '%s' failed" % expr, lineNum)
    ...
    def __run_pass1_insn(self, line, pos, lineNum):
        ...
        self.__pass1.append(
            (lineNum, opcode, modifier, amode, afield, bmode, bfield)
        )
        self.__insnNum += 1
    ...
    def __check_icws_88(self, opcode, modifier, amode, bmode, lineNum):
        ...
        if opcode in ("mul", "div", "mod", "seq", "sne", "nop", "ldp", "stp"):
            raise WarriorParseError(
                "Opcode %s not allowed under '88 rules" % opcode.upper(),
                lineNum,
            )
        if modifier:
            raise WarriorParseError(
                "Instruction modifiers are not allowed under '88 rules", lineNum
            )
        if opcode == "dat":
        ...
        while pos < len(expr):
            ...
            if var in self.__labels:
                ...
                if label in self.__equ_deps[label]["deps"]:
                    raise WarriorParseError(
                        "Label '%s' is recursively defined" % label,
    ...
    def __evaluate_expr(self, expr, lineNum, offset=0):
        ...
        while pos < len(expr):
            ...
            if var in self.__labels:
                ...
                raise WarriorParseError("Label '%s' is unknown" % var, lineNum)
            ...
            while not pos2 == pos1 + 2:
                ...
                for pos in range(pos1 + 1, pos2):
                    if self.__is_op(tokens[pos]):
                        ...
                        if tokens[pos - 1] == "(" or self.__is_op(
                            tokens[pos - 1]
                        ):
                            if tokens[pos] in "!-+":
                                op_prec = 7
